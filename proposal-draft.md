...
Vehicular crowdsensing systems rely on participating vehicles to report traffic and safety information, but the same incentive structure that makes large-scale participation possible also makes the system vulnerable to Sybil attacks, in which one adversary forges multiple identities to submit fabricated messages and claim illegitimate rewards. These attacks are difficult to detect because the forged messages can look locally plausible in position, speed, and heading while still exhibiting temporal inconsistencies, replay behavior, or unrealistic motion patterns over longer windows.

This thesis proposes RoadFM-Lite, a self-supervised foundation model that learns road-network-grounded trajectory representations from the VeReMi Sybil dataset and applies them to Sybil attack detection. The VeReMi benchmark is generated by SUMO, a microscopic traffic simulator that models vehicles moving on a real urban road network. The resulting Basic Safety Message traces—reporting position, speed, acceleration, and heading at each time step—are therefore inherently road-network-grounded: they reflect lane-following behavior, intersection dynamics, and network-constrained motion. RoadFM-Lite learns these road-level movement regularities implicitly through a lightweight Transformer encoder operating on fixed-length trajectory windows, without requiring an explicit road graph or map-matching pipeline as input. The foundation model is pretrained with two objectives: masked trajectory reconstruction (MTR), which learns normal short-horizon motion patterns, and trajectory consistency prediction (TCP), which trains the model to recognize synthetic replay, shuffling, speed-scaling, and offset perturbations generated directly from VeReMi windows.

The pretrained foundation model is evaluated on VeReMi under three paradigms: few-shot contrastive fine-tuning with $K \in \{5, 10, 20\}$ labeled examples per class, zero-shot retrieval-based detection using a benign memory bank, and fully supervised classification as an upper-bound reference. The evaluation is further extended with scenario-holdout experiments across the available VeReMi scenario groups to test whether the learned road-network-grounded representations generalize across different traffic conditions and attack realizations. Baselines include the same encoder trained from scratch, a feature-engineered Random Forest classifier, and a supervised BiLSTM sequence classifier. The expected contributions are a self-supervised trajectory foundation model for road-network-grounded representation learning on VeReMi, a data-efficient detection framework tailored to Sybil behavior, and a reproducible evaluation protocol for few-shot, zero-shot, and scenario-holdout Sybil detection.
...
- **Scenario Generalization:** Even within the VeReMi benchmark, different attack families and scenario groups expose the model to different traffic densities, timing windows, and attack realizations. A detector that memorizes one VeReMi split can still fail when evaluated on another split of the same benchmark.

- **Representation Limitations:** Existing Sybil detection pipelines on VeReMi are dominated by hand-crafted plausibility rules, RSSI signals, trust scores, or fully supervised classifiers. By contrast, self-supervised trajectory foundation models are designed to learn reusable motion representations from unlabeled sequences, but they have not been systematically adapted to raw VeReMi traces for Sybil detection.

- **Benchmark-to-Model Gap:** The raw VeReMi files are message-oriented rather than model-ready. They must be transformed into trajectory windows, aligned by sender pseudonym and time, and split carefully to avoid leakage between nearly adjacent windows. This preprocessing problem is central to the study but is not addressed by off-the-shelf security baselines.

### 1.2 Research Gap

The core research gap addressed in this thesis is the absence of a self-supervised trajectory foundation model that learns road-network-grounded representations for Sybil detection on vehicular benchmarks such as VeReMi. Most VeReMi studies either depend on hand-crafted heuristics or train directly on labeled attack data, while most trajectory foundation models are evaluated on generic mobility tasks rather than adversarial detection. Three practical gaps remain:

1. **No benchmark-centered self-supervised pipeline.** There is no clear proposal for converting raw VeReMi message traces into reusable trajectory windows and learning sequence representations from them before downstream Sybil detection.
...
3. **No systematic within-benchmark generalization study.** The VeReMi scenario families and time-group splits allow evaluation beyond a single train/test split, but this structure is rarely used to measure whether learned representations transfer across scenario conditions.

### 1.3 Central Hypothesis

Self-supervised pretraining on road-network-grounded VeReMi trajectory windows using reconstruction and consistency objectives will produce foundation model embeddings that transfer more effectively to Sybil detection than training the same encoder architecture from scratch. This benefit should be largest in two settings where current methods are weakest: (1) few-shot supervision, where labeled attack examples are limited to 5–20 per class, and (2) scenario-holdout evaluation, where the detector is trained on one VeReMi scenario group and tested on another.

The mechanistic claim is straightforward: a foundation model that has already learned how road-network-constrained VeReMi trajectories evolve over time, and that has been trained to identify corrupted sequence patterns such as replay, shuffling, and offset perturbations, should separate benign and Sybil behavior more efficiently than a model that sees the same data only through supervised labels.

### 1.4 Research Question

**Does a self-supervised trajectory foundation model trained on road-network-grounded VeReMi message traces produce representations that improve vehicular Sybil detection relative to supervised-only baselines, particularly under few-shot supervision and scenario-holdout evaluation?**

This question decomposes into four sub-questions:

1. Does self-supervised pretraining improve Sybil detection accuracy compared to the same sequence encoder trained from scratch on the same VeReMi splits?
...
3. Does the pretrained encoder reduce performance loss when evaluation moves from one VeReMi scenario group to another?
...
- **Scenario Robustness:** If the model generalizes across VeReMi scenario groups, it is less likely to depend on a single curated split and more likely to capture durable behavioral signals.
...
- **Reproducible Scope:** Building the foundation model on the VeReMi benchmark keeps the experimental scope focused and reproducible, and it makes the final pipeline and pretrained weights easier to release and reuse.

### 1.6 Scientific Context

This thesis sits at the intersection of three closely related research areas:

**Trajectory Self-Supervised Learning.** Recent work has shown that masked modeling, contrastive learning, and related self-supervised objectives can produce reusable trajectory embeddings without manual annotation [4, 5, 9, 10, 13]. The key open question for this thesis is whether those ideas can be adapted effectively to raw VeReMi message traces.

**Trajectory Anomaly and Consistency Modeling.** Sequence models are increasingly used to learn what normal motion looks like and to detect corrupted or implausible temporal patterns [28, 29, 30, 31, 32]. This thesis draws on that idea by using synthetic trajectory corruptions during pretraining, but targets Sybil detection rather than generic anomaly detection.

**Vehicular Sybil Detection.** The vehicular security literature provides the attack framing, benchmark setting, and strong traditional baselines [3, 6, 7, 8, 18, 19, 20, 33]. However, most of this literature still favors heuristics or fully supervised classifiers over reusable pretrained trajectory foundation models. This thesis targets that gap directly within the VeReMi benchmark.

## 2 Related Work

The proposed research builds on recent work in trajectory foundation models, self-supervised representation learning, and vehicular security. This section emphasizes self-supervised sequence learning, road-network-grounded trajectory modeling, Sybil detection on VeReMi, and low-label evaluation settings.
...
For this thesis, the main lesson from these works is not to replicate their scale, but to adapt the foundation model paradigm—pretrain on unlabeled trajectory data, then adapt to downstream tasks—to a security-oriented vehicular benchmark. RoadFM-Lite shares this paradigm but applies it to road-network-grounded VeReMi traces for Sybil detection rather than generic mobility analysis.
...
**Masked Modeling.** START [9] and related masked-trajectory methods learn useful motion representations by hiding part of a sequence and asking the encoder to reconstruct it. Multi-scale spatiotemporal pretraining [21] further shows that temporal structure at different granularities can be learned without manual labels.

**Contrastive and Multi-View Learning.** Contrastive SSL for large-scale trajectories [10], geography-aware Siamese models [22], MMTEC [13], and RED [23] all demonstrate that agreement across multiple views or augmentations can produce embeddings that transfer to downstream tasks.

These works are strong methodological priors for this thesis, but they are mostly optimized for generic trajectory semantics rather than Sybil-specific inconsistencies. In particular, they do not focus on replay behavior, local temporal reordering, or other corruption patterns that are especially relevant to fabricated vehicular messages. This is precisely where a VeReMi-centered consistency objective becomes useful.

### 2.3 Trajectory Anomaly and Consistency Modeling

Trajectory anomaly detection is related to Sybil detection because both tasks require identifying motion patterns that deviate from normal behavior. Graph-based anomaly detection [28], road-partition anomaly models [29], encoder-decoder methods for driving anomalies [30], bus-trajectory anomaly detection [31], and generative approaches to mobility anomalies [32] all show that temporal models can learn a notion of normality and then flag deviations.

However, Sybil behavior is not simply a generic outlier problem. A Sybil trajectory may be crafted to look locally plausible while still betraying replay artifacts, inconsistent timing, duplicated patterns, or systematic offsets over a window. That makes the task closer to sequence-consistency modeling than to unconstrained outlier detection. This thesis therefore borrows the idea of learning normal motion structure, but targets Sybil-specific corruptions generated directly from VeReMi windows.

### 2.4 Vehicular Sybil Detection and the VeReMi Benchmark

The vehicular security literature provides the attack framing and the benchmark context for this study. VeReMi [19] and the VeReMi Extension [20] established a reusable benchmark for VANET misbehavior detection. On top of that benchmark, prior work has explored plausibility checks combined with machine learning [3], RSSI-based methods [6], proof-of-work and location-based methods [7], collaborative learning [8], and pseudonym-based approaches such as P2DAP [18].
...
These settings are especially relevant for a VeReMi-based study because they let the thesis evaluate not just raw classification accuracy, but also label efficiency and representation quality. In other words, the model should not only classify known attacks well; it should also adapt quickly and detect suspicious behavior when supervision is weak.

### 2.6 Implicit Road-Network Grounding Through Simulation Data

Several trajectory-learning papers achieve road-network grounding by consuming explicit map assets, road graphs, or external corpora [11, 14, 15, 16, 17, 24, 25, 26, 27]. RoadFM-Lite takes a different approach: it achieves road-network-grounded representations implicitly because the VeReMi dataset is itself generated by SUMO, a microscopic traffic simulator that models vehicles on a real urban road network (the Luxembourg SUMO scenario). Every position, speed, heading, and acceleration value in VeReMi reflects road-constrained movement—vehicles follow lanes, decelerate at intersections, and turn according to the network topology.

This means the foundation model learns road-level structure from the trajectory data without requiring an explicit road graph as input. The approach is more reproducible, avoids dependencies on external map-matching pipelines, and keeps the experimental scope focused on the VeReMi benchmark while still producing trajectory representations that are grounded in road-network dynamics.

### 2.7 Research Gaps

Our analysis of the literature reveals four concrete gaps that this thesis addresses:

1. **Foundation Model Gap:** VeReMi-based Sybil detection still relies primarily on rules, trust metrics, signal-based checks, or supervised classifiers. Self-supervised trajectory foundation models that learn road-network-grounded representations for Sybil detection are largely absent.
...
3. **Scenario-Generalization Gap:** The local VeReMi data contains multiple scenario families and group splits such as `0709` and `1416`, yet this structure is rarely used to test whether learned representations transfer beyond one curated split.

4. **Preprocessing Gap:** Raw VeReMi traces are message-oriented rather than model-ready. There is no widely adopted benchmark-centered pipeline for turning them into leakage-safe trajectory windows for self-supervised learning and downstream detection.

### 2.8 Our Contribution

This thesis addresses those gaps through a focused and internally consistent contribution set. First, it proposes RoadFM-Lite as a self-supervised trajectory foundation model that learns road-network-grounded representations from VeReMi simulation data. Second, it introduces a pretraining design based on masked trajectory reconstruction and trajectory consistency prediction using synthetic corruptions derived directly from the benchmark. Third, it evaluates the resulting embeddings under few-shot, zero-shot, fully supervised, and scenario-holdout settings, thereby measuring both label efficiency and within-benchmark generalization. Finally, it defines a reproducible preprocessing pipeline that converts raw VeReMi messages into sender-aligned trajectory windows suitable for security-oriented sequence learning.

## 3 Proposed Architecture

The implementation of RoadFM-Lite follows an encoder-decoder foundation model design built directly on raw VeReMi messages. The architecture has four components: a preprocessing stage that converts line-delimited traces into fixed-length trajectory windows, a Transformer encoder that produces road-network-grounded representations from those windows, a Transformer decoder that reconstructs masked or corrupted trajectory segments from the encoder representations, and task-specific heads that change across self-supervised pretraining and downstream detection. During pretraining, the encoder and decoder are trained jointly; during downstream Sybil detection, only the encoder is retained and the decoder is discarded, following the standard foundation model paradigm.
...
Each VeReMi message is treated as one time step in a sender-aligned sequence. Let a raw message at time $t$ be
...
This representation is intentionally simple. It uses only information present in the local VeReMi data while still exposing the encoder to both absolute state and short-term temporal change.
...
The encoder-decoder foundation model is pretrained on unlabeled VeReMi windows using two complementary objectives. Both operate on the same backbone and require no external annotations. The overall pretraining loss is
...
TCP is deliberately closer to the downstream problem than a generic augmentation objective. It teaches the encoder to become sensitive to replay artifacts, temporal disorder, and persistent offsets that are plausible manifestations of Sybil-style fabrication in VeReMi traces.

## 5 Downstream Task: Sybil Detection

After pretraining, the decoder and self-supervised heads are discarded and the pretrained encoder is evaluated on Sybil detection tasks built from the same local VeReMi data. The central question is whether the road-network-grounded representations learned by the full encoder-decoder foundation model improve downstream discrimination and label efficiency compared to encoder-only training from scratch.
...
**Memory Bank Construction.** A memory bank $\mathcal{M}_{\text{benign}}$ is populated with embeddings from verified benign VeReMi windows in the training partition.
...
The proposed model is compared against three baselines that can all be trained using the same VeReMi inputs and splits.
...
This baseline provides a practical benchmark for how far simple, interpretable summary features can go on VeReMi without sequence pretraining.

### 6.3 Baseline 3: Supervised BiLSTM Sequence Classifier

This baseline feeds the same VeReMi feature sequence into a standard bidirectional LSTM followed by a classification head. It tests whether the benefits of RoadFM-Lite come from self-supervised pretraining rather than from sequence modeling alone.
...
All detection experiments report the following metrics on held-out VeReMi data:
...
Metrics are reported for both binary benign-versus-Sybil detection and per-family analysis across the available VeReMi attack families.

### 7.2 Leakage-Safe Split Protocol

Because VeReMi windows are extracted from continuous traces, preventing leakage is essential. The split protocol follows three rules:
...
The primary generalization study replaces the earlier cross-city plan with a within-benchmark scenario-holdout evaluation using only the local VeReMi data.
...
This experiment tests whether the representation captures durable trajectory structure rather than memorizing one particular VeReMi subset.
...
1. **Road-Network-Grounded Trajectory Foundation Model.** A self-supervised foundation model pipeline that converts raw VeReMi message traces into leakage-safe trajectory windows, learns road-network-grounded representations through a compact Transformer encoder, and supports downstream task adaptation without external data.

2. **Empirical Validation for Data-Efficient Sybil Detection.** Controlled experiments showing whether self-supervised pretraining improves few-shot, zero-shot, and fully supervised detection relative to strong VeReMi baselines.

3. **Scenario-Holdout Evaluation Protocol.** A benchmark-centered generalization study across the available VeReMi scenario groups, including reproducible split definitions and ablation settings.
...
This thesis produces a pretrained trajectory foundation model relevant to Thrust 2 of the Sybil Guard project, delivering road-network-grounded representations within a focused and reproducible VeReMi benchmark scope. Specifically:
